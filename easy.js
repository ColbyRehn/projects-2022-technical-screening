...
const altNumbers = (numArray) => {
    ...
    // Split postive and negative elements into their respective array.
    for (let i = 0; i < numArray.length; i++) {
        if (numArray[i] >= 0) {
            posArray[posArray.length] = numArray[i];
        } else  {
            negArray[negArray.length] = numArray[i];
        }
    }

    // Alternate postive and negative numbers into an array.
    // Prioritizes positive numbers if size of positive numbers is the same
    // as negative numbers.
    let addPos = (posArray.length >= negArray.length);
    let numPos = 0;
    let numNeg = 0;
    for (let j = 0; j < numArray.length; j++) {
        if (addPos) {
            sortedArray[j] = posArray[numPos];
            numPos++;
        } else {
            sortedArray[j] = negArray[numNeg];
            numNeg++;
        }
        addPos = !addPos;
    }
